...
> * **[Evidencia en el Código]:** Esto ya está programado en Leloir a través de los CRDs `Tenant` y `Budget` (m15), que rastrean tokens. La retención de contexto se resolvió implementando `harness.memory.server` (un servidor MCP nativo) que actúa como memoria de largo plazo, en lugar de un proxy ciego.
...
> * **[Evidencia en el Código]:** La arquitectura de Leloir fuerza a que las llamadas pasen por la "Fachada MCP" (el Gateway). Esto permite interceptar cualquier invocación de herramientas (tools). El CRD `Investigation` actúa como árbitro: si la tool solicitada viola el RBAC del Tenant, Leloir devuelve la simulación. 
...
> * **[Evidencia en el Código]:** Leloir resolvió esto delegando la capa de caché a **LiteLLM** (configurado en el alias de modelos). En lugar de escribir lógica de caché de cero (regla D7 no-reinventar), el control plane inyecta LiteLLM para manejar el Semantic Cache y fallback, bajando los tiempos drásticamente.
...
> * **[Evidencia en el Código]:** Esto se evidencia en la configuración del Helm Chart actual (commit 909e7c0): el `embeddingModel: "local"` direcciona a modelos internos, pero a través del alias de LiteLLM (una perilla de configuración, sin tocar código) se puede hacer swap hacia OpenAI/Anthropic de manera 100% transparente para el agente.
